...
|---|---|---|---|
| Secrets / authenticators — Desk half (§9.4, IA-05) | PAT never in git or SQLite. Read from process env / Codex `env_key` / optional OS secret slot / encrypted Desk env vault (`keystore.rs`, `env_vault.rs`, `src-preview/crypto.ts`). Setup / Env lists FOUND/MISSING. Setup refuses PAT-in-store, PAT-in-`config.toml`, and endpoint query tokens. Logs redact token-like lines. Vault values export only to child `codex` | **PASS** (product) | AO still owns PAT issuance/rotation and CAC-backed authenticators (row below) |
